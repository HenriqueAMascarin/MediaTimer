...
export default function HamburguerMenu({ initialOption }: { initialOption: themesType | null }) {
    ...
    return (
        <View style={hamburguerStyles.container}>
            <TouchableOpacity style={hamburguerStyles.hamburguerContainer} onPress={() => toggleModal()} aria-label={t('theme.btnConfigAria')}>
                ...
                <Animated.View style={{ flex: 1, flexGrow: 1, zIndex: 10, opacity: opacityModal }}>
                    ...
                    <Animated.View style={[hamburguerStyles.modalContainer, { backgroundColor: dataTheme.animatedValues.backgroundColor }]}>
                        ...
                        {typesTheme.map((theme, keyTheme) => {
                            return (
                                <TouchableOpacity onPress={() => onTheme(theme)} key={keyTheme} style={{ flexDirection: "row", alignItems: 'center', gap: 8 }} aria-label={`${t('theme.selectBtnAria')} ${theme.label}`}>
                                    <Animated.View style={{ borderWidth: 1, width: 18, borderRadius: 18, height: 18, position: 'relative', padding: 2, borderColor: dataTheme.animatedValues.principalColor }} aria-label={`${t('theme.text')} ${theme.label} ${!theme.isActive ? t('theme.notActive') : t('theme.active')}`}>
